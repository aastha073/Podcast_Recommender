...
class TestCollaborativeFilter:
    def test_fit_succeeds(self, trained_cf):
        # Updated for new SVD implementation
        assert trained_cf._svd is not None
        assert trained_cf._user_factors is not None
        assert trained_cf._item_factors is not None
    ...
    def test_cold_start_user(self, trained_cf, dataset):
        ...
        candidates = dataset.podcasts["podcast_id"].tolist()[:20]
        preds = trained_cf.predict_for_user("brand_new_user_xyz", candidates, is_new_user=True)
        assert len(preds) == 20
        assert preds["collab_score"].between(0.0, 1.0).all()
